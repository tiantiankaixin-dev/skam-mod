...
public class ThunderGodArmorRenderer implements net.fabricmc.fabric.api.client.rendering.v1.ArmorRenderer {

    private static final Identifier TEXTURE = new Identifier("skam", "textures/models/armor/thunder_god_armor.png");
    private FireGodArmorModel<LivingEntity> armorModel;

    @Override
    public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, ItemStack stack, LivingEntity entity, EquipmentSlot slot, int light, BipedEntityModel<LivingEntity> contextModel) {
        if (this.armorModel == null) {
            this.armorModel = new FireGodArmorModel<>(MinecraftClient.getInstance().getEntityModelLoader().getModelPart(SkamModClient.THUNDER_GOD_ARMOR_LAYER));
        }
